...
from modules.image_detection.settings_button import detect_settings_button
...
def detect_settings_button(image_path):
    ...
    settings_button_coords = (int(0.85 * width), int(0.80 * height),
                              int(0.98 * width), int(0.95 * height))
    ...
    mask_settings_button = cv2.inRange(hsv_settings_button, lower_gray,
                                       upper_gray)
    settings_button_detected = False
    contours, _ = cv2.findContours(mask_settings_button, cv2.RETR_EXTERNAL,
        ...
        if not settings_button_detected:
            error_embed = discord.Embed(
                ...
                ("🛑 **THIS'S NOT YOUR GAME PROFILE! STOP TRYING TO SNEAK IN**\n\n"
                 "📂 **Make sure to upload a valid screenshot of your game profile that includes the settings button.**"
                 ),
                color=discord.Color.red())
            error_embed.set_footer(
                text=
